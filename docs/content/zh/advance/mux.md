...
```stream_buffer```指定每条复用流的最大缓冲区大小（smux 流控窗口），单位为字节。默认值为4194304（4 MiB）。增大此值可提高单条流的吞吐量，但会增加内存占用；如自行设置，客户端和服务端必须保持一致。

```receive_buffer```指定每个smux会话的最大总接收缓冲区大小，单位为字节。默认值为4194304（4 MiB），且必须大于或等于```stream_buffer```；如自行设置，客户端和服务端必须保持一致。

```protocol```指定smux线协议版本，可选值为```1```和```2```（默认）。版本```1```兼容原版trojan-go（p4gefau1t）及Shadowrocket等iOS客户端；版本```2```加入滑动窗口以改进流控。客户端和服务端必须使用相同版本。
